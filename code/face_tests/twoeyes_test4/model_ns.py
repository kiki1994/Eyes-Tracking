...
class BaseMode(nn.Module):
    def __init__(self):
        ...
        self.conv5 = nn.Sequential(nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1, dilation=1),
                                   nn.BatchNorm2d(512),  # 33£¬30
                                   nn.ReLU())
        self.conv6 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1, dilation=1),
                                   nn.BatchNorm2d(512),  # 33£¬30
                                   nn.ReLU(),
                                   nn.MaxPool2d(3, stride=2)) #16, 14
        self.conv7 = nn.Sequential(nn.Conv2d(512, 1024, kernel_size=3, stride=1, padding=1, dilation=1),
                                   nn.BatchNorm2d(1024),
                                   nn.ReLU(),
                                   nn.MaxPool2d(3, stride=2))    # 7, 6
        self.conv8 = nn.Sequential(nn.Conv2d(1024, 1024, kernel_size=3, stride=1, padding=0, dilation=1),
                                   nn.BatchNorm2d(1024),
                                   nn.ReLU(),
                                   nn.MaxPool2d(3, stride=2))    # 2,1
        self.fc1 = nn.Sequential(
            nn.Linear(1024* 2 * 1, 2048),
            nn.ReLU())
