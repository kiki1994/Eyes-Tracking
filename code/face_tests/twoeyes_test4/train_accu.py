...
def train(epoch):
    ...
    with open(file_path, 'a+') as file:
        for i, (face_img, label_left, label_right, head_pose_left, head_pose_right, eye_loc, face_loc) in enumerate(train_loader):
            start = time.time()
            count +=1

            face_img = face_img.cuda(device_ids[0])

            head_pose_left = head_pose_left.cuda(device_ids[0])
            head_pose_right = head_pose_right.cuda(device_ids[0])
            label_left = label_left.cuda(device_ids[0])
            label_right = label_right.cuda(device_ids[0])
            eye_loc = eye_loc.cuda(device_ids[0])
            face_loc = face_loc.cuda(device_ids[0])

            optimizer.zero_grad()
            result = model(face_img, eye_loc, face_loc, head_pose_left, head_pose_right)       ##output 128 x 6
            result_l = result[:, :3]
            result_r = result[:, 3:]
...
def test():
    ...
    for i, (face_img, label_left, label_right, head_pose_left, head_pose_right, eye_loc, face_loc) in enumerate(test_loader):
        start = time.time()
