...
class BaseMode(nn.Module):
    ...
    def forward(self, x,  eyes_loc):
        add_points = torch.zeros([x.size()[0], 128, 1, 32]).cuda()
        points = torch.cat([eyes_loc], 1)
        #print(points.shape)
        points = points.view(x.size()[0], 1, 24)                     # torch.Size([128, 1, 6])
        points = points.unsqueeze(1)
        #print(points.shape)
        points = points.repeat([1, 128, 1, 1])                   #[128, 256, 1, 6]     
        #print(points.shape)                            
        add_points[:, :, :, :24] = points
        ...
        last_conv = self.conv7(self.conv6(self.conv5(self.conv4(new_tens))))     
