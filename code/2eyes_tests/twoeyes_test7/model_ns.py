...
class BaseMode(nn.Module):
    ...
    def forward(self, x,  left_headpose, right_headpose):
        add_points = torch.zeros([x.size()[0], 256, 1, 15]).cuda()
        points = torch.cat([left_headpose, right_headpose], 1)
        #print(points.shape)
        points = points.view(x.size()[0], 1, 6)                     # torch.Size([128, 1, 6])
        points = points.unsqueeze(1)
        #print(points.shape)
        points = points.repeat([1, 256, 1, 1])                   #[128, 256, 1, 6]     
        #print(points.shape)                            
        add_points[:, :, :, :6] = points
        ...
        last_conv = self.conv7(new_tens)         #torch.Size([1, 256, 1, 6])
