...
class BaseMode(nn.Module):
    def __init__(self):
        ...
        self.conv4 = nn.Sequential(nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=1, dilation=1),
                                   nn.BatchNorm2d(256),  # 12*32
                                   nn.ReLU())
        self.conv5 = nn.Sequential(nn.Conv2d(256, 384, kernel_size=3, stride=1, padding=1, dilation=1),
                                   nn.BatchNorm2d(384),  # 12*32
                                   nn.ReLU())
######################DSPP####################
        self.fc6_dspp = nn.Sequential(nn.Conv2d(384,384,kernel_size=1,stride=1,padding=0,dilation=1),
                         nn.BatchNorm2d(384),         # 12*32
                         nn.ReLU())
        self.fc7_dspp = nn.Sequential(nn.Conv2d(384,256, kernel_size=1,stride=1, padding=0, dilation =1),
                          nn.BatchNorm2d(256),        # 12*32
                          nn.ReLU())
        self.fc8_dspp = nn.Sequential(nn.Conv2d(256,1, kernel_size=1,stride=1, padding=0, dilation =1),
                          nn.BatchNorm2d(1),        # 12*32
                          nn.ReLU())
##############################################
        self.conv6 = nn.Sequential(nn.Conv2d(384, 256, kernel_size=3, stride=1, padding=1, dilation=1),
                                   nn.BatchNorm2d(256),
                                   nn.ReLU(),
                                   nn.MaxPool2d(3, stride=2))    # 5*15
    ...
    def forward(self, x, eyes_loc, face_loc, left_headpose, right_headpose):
        add_points = torch.zeros([x.size()[0], 128, 3, 32]).cuda()
        ...
        #print(points.shape)
        points = points.repeat([1, 128, 1, 1])                   #[128, 128, 3, 28]     
        #print(points.shape)                            
        ...
        before_dspp = self.conv5(self.conv4(new_tens))         #torch.Size([1, 256, 1, 6])
        after_dspp = self.fc8_dspp(self.fc7_dspp(self.fc6_dspp(before_dspp)))
        dspp_conv = before_dspp * after_dspp
        last_conv = self.conv7(self.conv6(dspp_conv))
