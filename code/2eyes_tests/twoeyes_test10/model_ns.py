...
class BaseMode(nn.Module):
    def __init__(self):
        ...
        self.conv6 = nn.Sequential(nn.Conv2d(384, 256, kernel_size=3, stride=1, padding=1, dilation=1),
                                   nn.BatchNorm2d(256),
                                   nn.ReLU(),
                                   nn.MaxPool2d(3, stride=2))    # 4*15   + 6*15
        self.conv7 = nn.Sequential(nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=0, dilation=1),
                                   nn.BatchNorm2d(256),          #8*13
                                   nn.ReLU(),
                                   nn.MaxPool2d(3, stride=2))    # 3*6
        self.fc1 = nn.Sequential(
            nn.Linear(256 * 3 * 6, 2048),
            nn.ReLU())
    ...
    def forward(self, x, eyes_loc, face_loc, left_headpose, right_headpose):
        add_points = torch.zeros([x.size()[0], 256, 6, 15]).cuda()
        points = torch.cat([eyes_loc, face_loc, left_headpose, right_headpose], 1)
        #print(points.shape)
        points = points.view(x.size()[0], 6, 14)                     # torch.Size([256, 6, 14])
        points = points.unsqueeze(1)
        #print(points.shape)
        points = points.repeat([1, 256, 1, 1])                   #[128, 128, 3, 28]     
        #print(points.shape)                            
        add_points[:, :, :, :14] = points
