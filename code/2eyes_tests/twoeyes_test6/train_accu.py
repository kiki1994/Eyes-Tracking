...
def train(epoch):
    ...
    with open(file_path, 'a+') as file:
        for i, (two_eyes_img, label_left, label_right, head_pose_left, head_pose_right, eye_loc) in enumerate(train_loader):
            start = time.time()
            count +=1
...
#            face_loc = face_loc.cuda()
            ...
            optimizer.zero_grad()
            result = model(two_eyes_img, eye_loc, head_pose_left, head_pose_right)       ##output 128 x 6
            result_l = result[:, :3]
            result_r = result[:, 3:]
...
def test():
    ...
    for i, (two_eyes_img, label_left, label_right, head_pose_left, head_pose_right, eye_loc) in enumerate(test_loader):
        start = time.time()
