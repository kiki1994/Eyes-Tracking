...
class BaseMode(nn.Module):
    def __init__(self):
        ...
        self.conv7 = nn.Sequential(nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, dilation=1),
                                   nn.BatchNorm2d(256),          # 4*15
                                   nn.ReLU(),
                                   nn.MaxPool2d(3, stride=2))    # 1*7
        self.fc1 = nn.Sequential(
            nn.Linear(256 * 1 * 7, 2048),
            nn.ReLU())
    ...
    def forward(self, x, eyes_loc,  left_headpose, right_headpose):
        add_points = torch.zeros([x.size()[0], 128, 1, 32]).cuda()
        points = torch.cat([eyes_loc, left_headpose, right_headpose], 1)
        #print(points.shape)
        points = points.view(x.size()[0], 1, 30)                     # torch.Size([128, 3, 28])
        points = points.unsqueeze(1)
        #print(points.shape)
        points = points.repeat([1, 128, 1, 1])                   #[128, 128, 3, 28]     
        #print(points.shape)                            
        add_points[:, :, :, :30] = points
        ...
        last_conv = self.conv7(self.conv6(self.conv5(self.conv4(new_tens))))         #torch.Size([1, 256, 1, 6])
