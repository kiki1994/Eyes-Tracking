...
H5_address = "/disks/disk0/linyuqi/dataset/data_gaze/h5_loc/h5_txtlist/"
...
def train(epoch):
    ...
    global count
    model.train()
    for i ,(face_img, label_left, label_right, head_pose_left, head_pose_right) in enumerate(train_loader):
        start = time.time()
        ...
        result = model(face_img)       ##output 128 x 6
#        print(result.shape)
        label_avg = (label_left+label_right)/2
#        print(label_avg.shape)
        loss = L1_loss(result,label_avg)        
        ...
        if count % 23 == 0:
            ...
            if test_accu < min_saver.accu_min:
                ...
                min_saver.ep = epoch
            acc = accuracy_text(result,label_avg)    #two eyes' acc average ([128]) 
            ...
            print('num_of_iter = {}      min =({}, {:3.3})     train_ARE_loss={}     train_E_loss= {}   acc = {}'.format(
...
def test():
    ...
    for i ,(face_img, label_left, label_right, head_pose_left, head_pose_right) in enumerate(test_loader):
