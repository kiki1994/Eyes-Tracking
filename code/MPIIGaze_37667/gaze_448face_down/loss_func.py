...
class loss_f_E(nn.Module):
    ...
    def forward(self, result_l, result_r,prob_l,prob_r,mat_b):
        ...
        L_E128 = -(mat_b *torch.acos(torch.clamp((L_E128_up/L_E128_down),min=-1,max=1))*torch.log(torch.clamp(prob_l,min=1e-30,max=0.999999)) +(1 - mat_b) * torch.acos(torch.clamp((L_E128_up/L_E128_down),min=-1,max=1))*(torch.log(torch.clamp(prob_r,min=1e-30,max=0.999999))))
