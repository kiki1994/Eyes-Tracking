...
H5_address = '/disks/disk0/linyuqi/dataset/data_gaze/eval_txts/'
Save_model_address = '/disks/disk0/linyuqi/model/gaze_448face_down/'
BatchSize = 128 
EPOCH = 50
lr_init = 0.0001
count=0
...
AR_up_model = nn.DataParallel(AR_up_model)
AR_up_model.cuda()
...
optimizer_AR = torch.optim.Adam(AR_up_model.parameters(),lr=lr_init)
...
#        print(image_right)
        ...
        result_AR = AR_up_model(eye_img_left, eye_img_right,head_pose_left,head_pose_right)       ##output 128 x 6
       ...
       # print(result_E)
        ...
        if count % 23 == 0:
            ...
            count, min_saver.iter, min_saver.accu_min, loss, L_E, elapsed, accuracy_avg))
...
def main():
    ...
    with open('./record.txt', 'r') as f:
        contents = f.read()
        print(contents)
        contents = contents.replace(person_num, person_num + ' %3.3f %d' % (min_saver.accu_min, min_saver.iter))
        f.close()
