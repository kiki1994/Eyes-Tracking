...
class BaseMode_face(nn.Module):
    def __init__(self):
        super(BaseMode_face, self).__init__()
        self.conv1 = nn.Sequential(nn.Conv2d(3, 96, kernel_size=11, stride=4, padding=0, dilation=1),
                                   nn.BatchNorm2d(96),
                                   nn.ReLU())
...
class AR_Net_up(nn.Module):
    def __init__(self):
        ...
        self.faceModel = BaseMode_face()                           # not sharing the weight
    ...
    def forward(self, full_face):
        imag_face = self.faceModel(full_face)

        fc_f = self.fc1(imag_face)
