...
Save_model_address = '/disks/disk0/linyuqi/model/gaze_2eye_ARE_loss/'
BatchSize = 128 
EPOCH = 30
vis = visdom.Visdom()
count=0
count_ts=0
train_iteration =np.ceil(200000/128)
test_iteration = np.ceil(13659/128)
...
#        L_E, L_AR, L_AR2 = loss_f(result_AR[:,:3],result_AR[:,3:],label_left,label_right,result_E[:,0],result_E[:,1])
        ...
        if count%20 ==0:    
            ...
            vis.line(X=x, Y=accu,update = 'append',
                    win = 'tr_accuracy ', opts=({'title':'train accuracy and loss'})) 
            #print('num_of_batch = {}    train_loss={}      time = {:.2f}       acc = {}'.format(count,loss,elapsed,accuracy_avg))
        
    accuracy = accuracy_all/train_iteration
    print(accuracy) 
    print("-----train-----")
...
#        loss = L1_loss(result_AR,label)
        ...
        if i%20 ==0:      
            ...
            vis.line(X=np.column_stack((x,x,x)), Y=torch.from_numpy(np.column_stack((accu,loss_e,loss_are))),update = 'append',
                     win = 'ts_accuracy and loss',opts=dict({'title':'test accuracy and loss'},legend=["acc","L_E","L_ARE"]))  
