...
H5_test_address = '/disks/disk0/linyuqi/dataset/data_gaze/eval_txts/'
H5_train_address = "/disks/disk0/linyuqi/dataset/data_gaze/Multiview_all/h5_txtlist/"
Save_model_address = "/disks/disk0/linyuqi/model/ARE_cross_data_30/"
BatchSize = 128 
EPOCH = 30
lr_init = 0.0001
count=0
...
test_list = dataset.load_all_h5(H5_test_address)
...
def train(epoch):
    ...
    global count
    AR_model.train()
    for i ,( eye_img_left, eye_img_right, label_left, label_right, head_pose_left, head_pose_right) in enumerate(train_loader):
        start = time.time()
...
#        print(image_right)
        ...
        result_AR = AR_model(eye_img_left, eye_img_right, head_pose_left, head_pose_right)       ##output 128 x 6
...
def test():
    ...
    for i ,( eye_img_left, eye_img_right, label_left, label_right, head_pose_left, head_pose_right) in enumerate(test_loader):
        start = time.time()
        #face_img = face_img.cuda()
        eye_img_left = eye_img_left.cuda()
        eye_img_right = eye_img_right.cuda()
...
def main():
    for epoch in range(1, EPOCH):
        ...
        if train(epoch) == 1:
            ...
            f.close()
            break
        torch.save({'epoch': epoch,'state_dict': AR_model.state_dict(), }, os.path.join(Save_model_address, 'checkpoint_{}.tar'.format(epoch)))
        adjust_learning_rate(optimizer_AR, epoch, lr_init)
        adjust_learning_rate(optimizer_E, epoch, lr_init)
    
    with open('./record.txt', 'w') as f:
        
        f.write("(accu = %3.3f iter = %d  epoch = %d)"%(min_saver.accu_min, min_saver.iter, min_saver.ep))
        f.close()
